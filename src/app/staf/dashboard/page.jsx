...
const PendingApprovals = ({ approvals, loading }) => (
  <div className="bg-white rounded-xl shadow-md p-6">
    ...
    <div className="space-y-3">
      {loading ? (
        ...
        <div className="text-center py-8 text-gray-500">
          ...
          <p className="font-medium">Semua buku sudah diapprove! 🎉</p>
        </div>
      ) : (
    ...
    {!loading && approvals.length > 0 && (
      <button 
        onClick={() => window.location.href = '/staf/buku'}
...
export default function StafDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      console.log('📊 Fetching staf dashboard data...');
      
      // Fetch semua data secara parallel
      const [booksRes, usersRes, peminjamanRes] = await Promise.all([
        fetch('/api/staf/buku'),
        fetch('/api/staf/users'),
      ...
      const pendingBooksFiltered = booksArray.filter(b => b.status === 'pending');

      setStats({
        totalBuku: approvedBooks.length,
        totalUsers: usersArray.length,
        bukuDipinjam: peminjamanArray.filter(p => p.status === 'dipinjam').length,
        bukuPending: pendingBooksFiltered.length
      });
      ...
      // Set pending books untuk display (max 5)
      setPendingBooks(pendingBooksFiltered.slice(0, 5));
  ...
  const generateActivities = (peminjaman, books, users) => {
    ...
    // Sort peminjaman by updated_at or created_at (yang terbaru)
    ...
    // Tambahkan aktivitas buku baru jika ada
    ...
    recentBooks.forEach(book => {
      activities.push({
        icon: BookOpen,
        title: `Buku baru "${book.judul}" ditambahkan ke katalog`,
        time: formatTimeAgo(book.created_at),
        color: 'bg-indigo-500'
      });
    });

    // Tambahkan aktivitas user baru
  ...
  return (
    <div>
      ...
      {/* Activity, Pending Books, and Peminjaman Stats */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <RecentActivity activities={recentActivities} loading={loading} />
        <PendingApprovals approvals={pendingBooks} loading={loading} />
